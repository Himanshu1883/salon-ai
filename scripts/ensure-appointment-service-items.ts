...
const SQL_FILE = resolve(
  __dirname,
  "sql/ensure-appointment-service-items.sql"
);
...
async function ensureViaPg(
  ...
  try {
    ...
    console.log("Ensured AppointmentServiceItem table and backfill.");
  } finally {
    await client.end();
  }
}
...
function ensureViaPrismaCli() {
  ...
  console.log("Applied AppointmentServiceItem schema via Prisma CLI.");
}
...
main().catch((error) => {
  console.error("Failed to ensure AppointmentServiceItem:", error);
  process.exit(1);
});
