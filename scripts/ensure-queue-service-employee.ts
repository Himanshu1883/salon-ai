...
const SQL_FILE = resolve(__dirname, "sql/ensure-queue-service-employee.sql");
...
async function ensureViaPg(
  ...
  try {
    ...
    if (ready) {
      console.log("QueueService.employeeId already present.");
      return;
    }
    ...
    console.log("Applied QueueService.employeeId column.");
  } finally {
    await client.end();
  }
}
...
function ensureViaPrismaCli() {
  ...
  console.log("Applied QueueService.employeeId via Prisma CLI.");
}
...
async function main() {
  ...
  const ssl = requiresRailwaySsl(url)
...
main().catch((error) => {
  console.error("Failed to ensure QueueService.employeeId:", error);
  process.exit(1);
});
