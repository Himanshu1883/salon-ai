    ...
    <div className="divide-y divide-[#E8ECF4]">
      {customers.map((customer) => {
        ...
        return (
          <div
            key={customer.id}
            className="flex gap-2 p-2.5 sm:gap-3 sm:p-4"
            ...
            <div className="min-w-0 flex-1 space-y-1.5">
              <div className="flex items-start justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#6C3BFF]/15 to-[#FF2D6F]/15 text-[10px] font-semibold text-[#6C3BFF]">
                    {getInitials(customer.name)}
                  </div>
                  <div className="min-w-0">
                    <Link
                      href={getClientDetailPath(pathname, customer.id)}
                      className="block truncate text-sm font-semibold text-[#1C103D]"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {customer.name}
                    </Link>
                ...
                <Badge
                  className={cn(
                    "h-5 shrink-0 rounded-md border-0 px-1.5 text-[10px] font-medium",
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px]">
                <div>
                  <p className="text-[10px] text-[#9CA3AF]">Last visit</p>
                  <p className="font-medium text-[#374151]">
                    {formatLastVisit(customer.lastVisit)}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-[#9CA3AF]">Lifetime</p>
                  <p className="font-medium tabular-nums text-[#1C103D]">
                    {formatCurrency(customer.totalSales)}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-[#9CA3AF]">Avg ticket</p>
                  <p className="tabular-nums text-[#374151]">
                    {avgTicket !== null ? formatCurrency(avgTicket) : "—"}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-[#9CA3AF]">Loyalty</p>
                  <p className="font-medium tabular-nums text-[#1C103D]">
              ...
              <div
                className="flex justify-end border-t border-[#E8ECF4] pt-1.5"
                onClick={(e) => e.stopPropagation()}
                onKeyDown={(e) => e.stopPropagation()}
              >
...
export function ClientsTable({
  ...
  if (customers.length === 0) {
    return (
        <div className="overflow-hidden rounded-xl border border-[#E8ECF4] bg-white shadow-[0_4px_24px_rgba(28,16,61,0.05)] sm:rounded-[20px]">
        <ClientsEmptyState
          isLoading={isPending}
          onAdd={onAdd}
          onImport={onImport}
